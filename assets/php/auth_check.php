...
class OneFileLoginApplication
{
	...
	private function createNewUser()
	{
		...
		if ($result_row) {
			...
			if ($registration_success_state) {
				$this->feedbacksuccess = '<b>User credentials have been created successfully.</b><br><b><a href="settings.php" class="btn btn-primary" title="Monitorr Settings">Log in here</a> </b>';
				return true;
			} else {
	...
	private function doLogout()
	{
		...
		unset($_COOKIE["Monitorr_AUTH"]);
		setcookie("Monitorr_AUTH", null, time() - 1, "/");
	...
	private function performUserLoginAction()
	{
		...
		} elseif (isset($_COOKIE["Monitorr_AUTH"])) {
	...
	private function doLoginWithSessionData()
	{
		...
		if (!isset($_COOKIE['Monitorr_AUTH'])) {
			if ($this->createDatabaseConnection()) {
				...
				if ($result_row) {
					...
					setcookie("Monitorr_AUTH", $cookie_value, time() + 60 * 60 * 24 * 7, "/"); //store login cookie for 7 days
					return true;
				} else {
	...
	private function doLoginWithCookieData()
	{
		if ($this->createDatabaseConnection()) {
			...
			$sql = 'SELECT user_name, user_email, auth_token
                ...
                LIMIT 1';
			$query = $this->db_connection->prepare($sql);
			$query->bindValue(':auth_token', $_COOKIE['Monitorr_AUTH']);
			$query->execute();
			$result_row = $query->fetchObject();
			if ($result_row) {
				// write user data into PHP SESSION [a file on your server]
